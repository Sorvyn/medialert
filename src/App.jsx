...
export default function App() {
  ...
  const handleEmergency = async (item) => {
    ...
    try {
      const text = await callAPI(
        "Give ONLY numbered steps. First step must be CALL 1122 (Pakistan) or 112 (India). No theory. Just actions.",
  ...
  const handleMedicine = async (item) => {
    ...
    try {
      const text = await callAPI(
        "Give OTC medicine names with dosage, 3 home remedies, warning signs. Keep it simple.",
        `Treatment for: ${item.label}`
      )
      setResponse(text)
    } catch { setResponse("⚠️ Connection error. Try again.") }
    setLoading(false)
  ...
  const handleCustom = async () => {
    ...
    const isEmergency = /emergency|accident|bleeding|attack|unconscious|faint/.test(customQuery.toLowerCase())
    ...
    try {
      const system = isEmergency
        ? "Give ONLY numbered action steps. First step: CALL 1122 (PK) / 112 (IN)."
        : "Give medicine names with dosage and home remedies. Keep it simple."
  ...
  const checkSymptoms = async () => {
    ...
    try {
      const text = await callAPI(
        "List 2-3 possible conditions, urgency level (Emergency/Moderate/Mild), immediate steps.",
        `Symptoms: ${all.join(", ")}`
      )
  ...
  const findHospitals = async () => {
    ...
    try {
      const text = await callAPI(
        "Return ONLY a JSON array of hospitals. No text outside JSON. Format: [{name,address,phone,emergency}]",
        `${hospitalType} hospitals in ${hospitalLocation}`
      )
      const cleaned = text.replace(/```json|```/g, "").trim()
      setHospitals(JSON.parse(cleaned))
    } catch { setHospitals([{ name: "Try 'Lahore, Pakistan'", address: "Use a more specific location", phone: "", emergency: false }]) }
  ...
  const S = {
    page: { minHeight: "100vh", background: "#0a0a0f", color: "#fff", fontFamily: "system-ui, sans-serif", paddingBottom: 80 },
    ...
    title: { fontSize: 32, fontWeight: 800, margin: 0, letterSpacing: 2, color: "#fff" },
    ...
    card: (color) => ({ background: `linear-gradient(145deg, ${color}15, ${color}05)`, border: `1px solid ${color}30`, borderRadius: 14, padding: "14px 12px", color: "#fff", cursor: "pointer", textAlign: "left" }),
    ...
    btn: (color) => ({ width: "100%", background: `linear-gradient(135deg, ${color}, ${color}cc)`, border: "none", borderRadius: 12, color: "#fff", padding: "12px", fontSize: 13, fontWeight: 700, cursor: "pointer", marginTop: 10 }),
    result: { background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, padding: "14px", marginTop: 12, whiteSpace: "pre-wrap", fontSize: 13, lineHeight: 1.9, color: "rgba(255,255,255,0.9)" },
    overlay: { position: "fixed", inset: 0, zIndex: 100, background: "rgba(0,0,0,0.9)", display: "flex", alignItems: "center", justifyContent: "center" },
    modal: { background: "#1a0505", border: "2px solid #FF3B47", borderRadius: 20, padding: "28px 20px", textAlign: "center", maxWidth: 300, width: "92%" },
    bottomNav: { position: "fixed", bottom: 0, left: 0, right: 0, background: "rgba(10,10,15,0.97)", borderTop: "1px solid rgba(255,255,255,0.07)", padding: "10px 8px 16px", display: "flex", justifyContent: "space-around" },
    navBtn: (active) => ({ background: "transparent", border: "none", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, cursor: "pointer", opacity: active ? 1 : 0.4 }),
    ...
    <div style={{ ...S.page, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ maxWidth: 400, width: "100%" }}>
        <div style={{ textAlign: "center", marginBottom: 20 }}>
          ...
          <h1 style={{ ...S.title, fontSize: 32, display: "block" }}>MEDIALERT</h1>
          <p style={{ color: "rgba(255,255,255,0.4)", fontSize: 12, marginTop: 4 }}>AI FIRST AID</p>
        </div>
        <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 14, padding: "14px", marginBottom: 14, maxHeight: 280, overflowY: "auto" }}>
          ...
          {["NOT medical advice — always call 1122 (PK) / 112 (IN)", "AI may be incorrect — see a real doctor for diagnosis", "No data stored — requests are not saved", "No personal info collected", "No liability — use at your own risk"].map((t, i) => (
            <p key={i} style={{ fontSize: 12, color: "rgba(255,255,255,0.65)", margin: "7px 0" }}>✓ {t}</p>
          ))}
        </div>
    ...
    <div style={S.page}>
      {showSOS && (
        <div style={S.overlay}>
          <div style={S.modal}>
            ...
            <h2 style={{ color: "#FF3B47", fontSize: 22, margin: "0 0 12px" }}>EMERGENCY</h2>
            <p style={{ fontSize: 13, color: "rgba(255,255,255,0.8)", lineHeight: 2, marginBottom: 14 }}>
              🇵🇰 Rescue: <strong>1122</strong><br/>
              🇵🇰 Ambulance: <strong>115</strong><br/>
              🇵🇰 Police: <strong>15</strong><br/>
      ...
      {tab === "symptoms" && (
        <div style={S.content}>
          ...
          <input value={customSymptom} onChange={e => setCustomSymptom(e.target.value)} placeholder="+ Add custom symptom" style={{ ...S.input, marginBottom: 8 }} />
          <button onClick={checkSymptoms} disabled={!selectedSymptoms.length && !customSymptom} style={{ ...S.btn("#A78BFA"), opacity: (!selectedSymptoms.length && !customSymptom) ? 0.5 : 1 }}>🔍 Analyze Symptoms</button>
          {symptomLoading && <p style={{ textAlign: "center", color: "rgba(255,255,255,0.5)", fontSize: 12, marginTop: 12 }}>Analyzing...</p>}
          {symptomResult && !symptomLoading && <div style={S.result}>{symptomResult}</div>}
        </div>
      )}
      ...
      {tab === "hospitals" && (
        <div style={S.content}>
          ...
          <button onClick={findHospitals} disabled={!hospitalLocation.trim()} style={{ ...S.btn("#34D399"), opacity: !hospitalLocation.trim() ? 0.5 : 1 }}>🏥 Search</button>
          {hospitalLoading && <p style={{ textAlign: "center", color: "rgba(255,255,255,0.5)", fontSize: 12, marginTop: 12 }}>Searching...</p>}
            ...
            <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 10 }}>
              {hospitals.map((h, i) => (
                <div key={i} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(52,211,153,0.15)", borderRadius: 12, padding: "12px" }}>
                  ...
                  <div style={{ fontSize: 11, color: "rgba(255,255,255,0.5)", marginBottom: 8 }}>📍 {h.address}</div>
                  <a href={`https://www.google.com/maps/search/${encodeURIComponent(h.name + " " + h.address)}`} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, color: "#34D399", fontWeight: 700, textDecoration: "none" }}>🗺️ Open Maps</a>
                </div>
              ))}
            </div>
          )}
      ...
      {tab === "result" && selected && (
        <div style={S.content}>
          ...
          <div style={{ background: `${selected.color}15`, border: `1px solid ${selected.color}30`, borderRadius: 14, padding: "14px", marginBottom: 10 }}>
            <div style={{ fontSize: 30, marginBottom: 6 }}>{selected.icon}</div>
            <div style={{ fontSize: 18, fontWeight: 800 }}>{selected.label.toUpperCase()}</div>
          </div>
          {loading && <p style={{ textAlign: "center", color: "rgba(255,255,255,0.5)", fontSize: 12, padding: 20 }}>Loading...</p>}
          {response && !loading && (
            <div style={S.result}>
              {response}
              {selected.type === "emergency" && (
                <div style={{ marginTop: 14, background: "rgba(255,59,71,0.1)", borderRadius: 10, padding: "10px", textAlign: "center" }}>
                  <p style={{ fontSize: 12, margin: 0 }}>🚨 <strong style={{ color: "#FF3B47" }}>1122</strong> 🇵🇰 · <strong style={{ color: "#FF3B47" }}>112</strong> 🇮🇳</p>
                </div>
              )}
            </div>
          )}
      ...
      <div style={S.bottomNav}>
        {[["home","🏠","Home"],["emergency","🚨","SOS"],["medicine","💊","Meds"],["symptoms","🔍","Check"],["hospitals","🏥","Hospitals"]].map(([id, icon, label]) => (
